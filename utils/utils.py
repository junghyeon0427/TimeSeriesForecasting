...
def time_slide_df(df, window_size, forcast_size, date, target):
    ...
    for idx in range(0, df_.shape[0] - window_size - forcast_size + 1):
        ...
        y = df_.loc[
            idx + window_size : idx + window_size + forcast_size - 1, target
        ].values
        date_ = df_.loc[
            idx + window_size : idx + window_size + forcast_size - 1, date
        ].values
        data_list.append(x)
        dap_list.append(y)
        date_list.append(date_)
    return (
        np.array(data_list, dtype="float32"),
        np.array(dap_list, dtype="float32"),
        np.array(date_list),
    )
